...
import { closeTicket, addMessage } from '../store/slices/supportSlice';
...
const ChatMessage = () => {
    ...
    const activeMessages = messages[activeTicketId] || [];
    ...
    const handleSendMessage = () => {
        ...
        dispatch(addMessage({ ticketId: activeTicketId, message: newMessage }));
        setInput('');
    };

    const handleCloseTicket = () => {
        dispatch(closeTicket(activeTicketId));
    };
        ...
        <div className="h-[calc(100vh-160px)] flex animate-fadeIn overflow-hidden rounded-[40px] border border-brand-primary/5 shadow-soft bg-white">
            ...
            <div className="w-1/3 border-r border-brand-primary/5 flex flex-col overflow-hidden">
                <div className="p-6 border-b border-brand-primary/5 bg-brand-primary/[0.02]">
                    <h3 className="text-lg font-black text-brand-primary uppercase tracking-tight mb-4">Support Center</h3>
                    ...
                    {tickets.map((ticket) => (
                        ...
                        >
                            <div className="flex justify-between items-start mb-1">
                                <h4 className={`text-sm font-black truncate max-w-[140px] ${activeTicketId === ticket.id ? 'text-white' : 'text-brand-primary'}`}>
                                    {ticket.fullName}
                                </h4>
                                <span className={`text-[8px] font-bold uppercase tracking-widest ${activeTicketId === ticket.id ? 'text-white/40' : 'text-brand-primary/20'}`}>
                                    {formatDate(ticket.createdAt)}
                                </span>
                            </div>
                            <p className={`text-[10px] font-medium truncate mb-3 ${activeTicketId === ticket.id ? 'text-white/60' : 'text-brand-primary/40'}`}>
                                {ticket.company.name} • {ticket.subject}
                            </p>
                            <div className="flex items-center gap-2">
                                <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest uppercase border ${activeTicketId === ticket.id
                                    ? 'bg-white/10 text-white border-white/20'
                                    : 'bg-brand-primary/5 text-brand-primary/60 border-brand-primary/10'
                                    }`}>
                                    {ticket.priority}
                                </span>
                                <span className={`px-2 py-0.5 rounded-lg text-[8px] font-black tracking-widest uppercase ${ticket.status === 'OPEN'
                                    ? activeTicketId === ticket.id ? 'bg-white/20 text-white' : 'bg-green-100 text-green-700'
                                    : activeTicketId === ticket.id ? 'bg-brand-accent/20 text-brand-accent' : 'bg-brand-primary/10 text-brand-primary/40'
                                    }`}>
                                    {ticket.status}
                                </span>
                            </div>
            ...
            <div className="flex-1 flex flex-col bg-brand-primary/[0.01]">
                ...
                <div className="p-6 border-b border-brand-primary/5 flex items-center justify-between bg-white z-10 shadow-sm">
                    <div className="flex items-center gap-4">
                        <div className="w-12 h-12 bg-brand-primary rounded-xl flex items-center justify-center text-white ring-4 ring-brand-primary/5 shadow-soft overflow-hidden">
                            {activeTicket?.company.logo ? (
                                <span className="text-lg font-black uppercase">{activeTicket.company.name.charAt(0)}</span>
                            ) : (
                        ...
                        <div>
                            <div className="flex items-center gap-2">
                                <h3 className="text-base font-black text-brand-primary">{activeTicket?.fullName}</h3>
                                <div className="px-2 py-0.5 bg-brand-primary/5 rounded-md text-[8px] font-black text-brand-primary/30 uppercase tracking-widest flex items-center gap-1">
                                    <ShieldCheck className="w-2.5 h-2.5" /> ID: #{activeTicket?.id}
                                </div>
                            </div>
                            <p className="text-[10px] font-bold text-brand-primary/40 uppercase tracking-[0.1em] mt-0.5">
                                {activeTicket?.company.name} • Professional Support
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-3">
                        <button className="p-3 text-brand-primary/30 hover:bg-brand-primary/5 rounded-xl transition-colors"><Flag className="w-5 h-5" /></button>
                        <button className="p-3 text-brand-primary/30 hover:bg-brand-primary/5 rounded-xl transition-colors"><MoreHorizontal className="w-5 h-5" /></button>
                ...
                <div className="flex-1 overflow-y-auto p-8 space-y-6 flex flex-col no-scrollbar bg-[radial-gradient(#005A5B10_1px,transparent_1px)] bg-size-[24px_24px]">
                    {activeMessages.map((msg, idx) => (
                        <div key={idx} className={`flex flex-col ${msg.isRepliedByAdmin ? 'items-end' : 'items-start'}`}>
                            <div className={`max-w-[75%] p-5 rounded-[28px] ${msg.isRepliedByAdmin ? 'bg-brand-primary text-white rounded-tr-none shadow-premium' : 'bg-white border border-brand-primary/5 text-brand-primary rounded-tl-none shadow-soft'}`}>
                                <p className="text-sm font-medium leading-relaxed">{msg.content}</p>
                            </div>
                            <div className="flex items-center gap-1.5 mt-2.5 px-1">
                                {msg.isRepliedByAdmin && <CheckCheck className="w-3 h-3 text-brand-accent" />}
                ...
                <div className="p-6 bg-white border-t border-brand-primary/5 z-10">
                    <div className="flex items-center gap-4 bg-brand-primary/[0.03] p-2 rounded-2xl focus-within:bg-white focus-within:ring-4 focus-within:ring-brand-primary/5 transition-all border border-transparent focus-within:border-brand-primary/10">
                        <input
                            type="text"
                            placeholder={activeTicket?.status === 'OPEN' ? "Type your response to the user..." : "Ticket is closed. Re-open to send message."}
                    ...
                    <div className="flex items-center gap-4 mt-4 px-2">
                        ...
                        <button className="text-[9px] font-black text-brand-primary/40 hover:text-white hover:bg-brand-primary uppercase tracking-widest bg-brand-primary/[0.02] px-3 py-1.5 rounded-lg border border-brand-primary/5 transition-all">Check Subscription</button>
                        <button className="text-[9px] font-black text-brand-primary/40 hover:text-white hover:bg-brand-primary uppercase tracking-widest bg-brand-primary/[0.02] px-3 py-1.5 rounded-lg border border-brand-primary/5 transition-all">Transfer to Tech</button>
                    </div>
                </div>
            </div>
        </div>
